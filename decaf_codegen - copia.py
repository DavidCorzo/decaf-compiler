...
class codegen:
    ...
    def get_var(self, var_name):
        ...
        while (index != 0):
            if self.scope_stack[index].get(var_name):
                return (self.scope_stack[index][var_name], index) # var attributes & the scope in which it was found
            index -= 1
        if self.method_arguments[self.current_method].get(var_name):
            return (self.method_arguments[self.current_method][var_name], (CALLEE_OFFSET+(self.scope_space[len(self.scope_stack)-1])))
    ...
    def expr(self, children) -> str:
        ...
        if (productions == ['%id%', '<subscript>']):
            ...
            temp_reg = occupy_temp_reg()
            if var_attr[VAR_TYPE] == 'int':
                self.append_instructions(self.current_method, [instruction(f'lw {temp_reg} {variable_offset}($sp)')])
            elif var_attr[VAR_TYPE] == 'boolean':
                self.append_instructions(self.current_method, [instruction(f'lb {temp_reg} {variable_offset}($sp)')])
            else:
            ...
            if (bin_op_parent == '<arith_op>'):
                ...
                codegen_std_error(f'production not found for bin_op_child {bin_op_child}')
            ...
            self.append_instructions([
                instruction(f'nor {expr_reg} {expr_reg} {expr_reg}')
            ])
            return expr_reg
        elif (productions == ['(', '<expr>', ')']):
            EXPR_POS = 1
